...
static char *test_p2j_get_message_by_name3() {
    json_error_t error;
    json_t *meta_desc = json_loads((char *)p2j_meta_descriptor_json, 0, &error);
    mu_assert(meta_desc);

    json_t *message = p2j_get_message_by_name(meta_desc, ".google.protobuf.FieldDescriptorProto.Label", &error);
    mu_assert(message);

    mu_assert(json_is_object(message));
    //mu_print_json(message);
    const char *name = json_string_value(json_object_get(message, "name"));
    mu_assert_equal_str(name, "Label");
    //free(name);
...
static char *test_p2j_protobuf2json_object() {
    ...
    // check that the JSON generated is the same as the JSON generated by NodeJS's protobuf package
    char *json_cstring = json_dumps(result, JSON_ENCODE_ANY|JSON_INDENT(2)|JSON_PRESERVE_ORDER);

    #define max(a, b) ((a) > (b) ? (a) : (b))
    for (size_t i = 0; i < max(strlen(json_cstring), strlen((char *)p2j_meta_descriptor_json)); i++) {
    	int a = i < strlen(json_cstring) ? json_cstring[i] : -1;
    	int b = i < strlen((char *)p2j_meta_descriptor_json) ? ((char *)p2j_meta_descriptor_json)[i] : -1;
    	if (a != b) {
    		fprintf(stderr, "diff at character %zu, a = %c(%i), b = %c(%i)\n", i, a, a, b, b);
    	}
    }

    mu_assert_equal_str(json_cstring, (char *)p2j_meta_descriptor_json);
...
static char *all_tests() {
    ...
    mu_run_test(test_p2j_get_message_by_name3);
